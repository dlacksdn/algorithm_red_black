...
struct CourseTree {
    ...
    pair<int, int> insertOrUpdate(int sid, const string& subject,
        const string& name, int semester,
        const string& phone, long long ts) {
        Key k{ sid, subject };
        ...
        while (x != Nil) {
            ...
            if (k == x->key) { // 중복이 존재
                ...
                int d = 0;
                Node* p = x;
                while (p->parent != Nil) { d++; p = p->parent; }
                return { d, 1 };
            }
            if (k < x->key) x = x->left;
            else            x = x->right;
    ...
    // search by sid: collect all matching into vec
    void collectBySid(Node* x, int sid, vector<Node*>& vec) {
        if (x == Nil) return;
        if (sid < x->key.sid) {
            collectBySid(x->left, sid, vec);
        }
        else if (sid > x->key.sid) {
            collectBySid(x->right, sid, vec);
        }
        else {
    ...
    void collectBySubject(Node* x, const string& subject,
        int depth, vector<Node*>& vec, long long& sumDepth) {
        if (x == Nil) return;
        if (x->key.subject == subject) {
            vec.push_back(x);
            sumDepth += depth;
        }
        collectBySubject(x->left, subject, depth + 1, vec, sumDepth);
        collectBySubject(x->right, subject, depth + 1, vec, sumDepth);
    }
    ...
    vector<pair<string, char>> listStudent(int sid) {
        ...
        sort(tmp.begin(), tmp.end(),
            [](Node* a, Node* b) { return a->key.subject < b->key.subject; });
        vector<pair<string, char>> out;
        out.reserve(tmp.size()); // 요소 수 많으면 성능 향상
        for (auto* n : tmp)
            out.emplace_back(n->key.subject,
                n->color == RED ? 'R' : 'B');
        return out;
    }
    ...
    vector<pair<int, char>> topKByTimestamp(const string& subj, int K) {
        ...
        sort(tmp.begin(), tmp.end(),
            [](Node* a, Node* b) { return a->timestamp < b->timestamp; });
        vector<pair<int, char>> out;
        int limit = min(K, (int)tmp.size());
        out.reserve(limit);
        for (int i = 0; i < limit; ++i) {
            Node* n = tmp[i];
            out.emplace_back(n->key.sid,
                n->color == RED ? 'R' : 'B');
        }
        return out;
    }
